...
/** Platforms we can store in settings (includes upcoming ones). */
...
/** Platforms with live chat ingest wired today (keep in sync with API). */
...
export type ParsedChannel = {
  ...
  handle: string;
  /** Present when input was a live/watch URL — resolve to @handle before saving */
...
function extractYoutubeVideoId(parts: string[], url: URL): string | null {
  const first = parts[0] ?? "";
  if (first === "live" && parts[1] && isYoutubeVideoId(parts[1])) {
    return parts[1];
  }
  if (first === "watch") {
...
export function parseChannelInput(input: string): ParsedChannel | { error: string } {
  ...
  return {
    error:
      "Paste a channel link (youtube.com/@name, youtube.com/live/…, twitch.tv/name, kick.com/name, rumble.com/c/name) or use platform/name",
  };
}
